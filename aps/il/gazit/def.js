export const APS = 'gazit'
export const PORT = 49043
export const HOST =
  process.env.NODE_ENV !== 'production'
    ? process.env.DEVELOPMENT_SERVER
    : process.env.PRODUCTION_SERVER
export const HTTP = 9040
const POLL_TIME = 600
export const PLC = {
  ip: '140.80.38.12',
  rack: 0,
  slot: 2,
  polling_time: POLL_TIME
}

export const QUEUE_LEN = 5

export const ALARM_LEN = 8
export const DB_ALARM_INIT = 6
export const DB_ALARM_LEN = 64 * ALARM_LEN
export const DBS_ALARM = [531, 532] // EL, EU

// const CARDS = 49
// const CARD_LEN = 10
export const CARDS = 49
export const CARD_LEN = 10

const DB_DATA = 505
const DB_DATA_LEN = 128
export const DB_DATA_INIT_DEVICE = 32
export const DB_DATA_INIT_POS = 64
export const DB_DATA_INIT_QUEUE = 76
export const DB_DATA_INIT_AB = 96
export const DB_DATA_INIT_EB = 106
export const DB_DATA_INIT_MB = 120
export const DATA_READ = {
  area: 0x84,
  dbNumber: DB_DATA,
  start: 0,
  amount: DB_DATA_LEN,
  wordLen: 0x02
}

// const STALLS = 50
// const STALL_LEN = 10
export const STALLS = 50
export const STALL_LEN = 10
export const STALL_STATUS = {
  FREE: 0,
  PAPA: 997,
  RSVD: 998,
  LOCK: 999
}

export const CARD_READ = {
  area: 0x84,
  dbNumber: 511,
  start: 0,
  amount: CARDS * CARD_LEN,
  wordLen: 0x02
}
export const CARD_EDIT = {
  area: 0x84,
  dbNumber: DB_DATA,
  start: 134,
  amount: 4,
  wordLen: 0x02
}
export const MAP_READ = {
  area: 0x84,
  dbNumber: 510,
  start: 0,
  amount: STALLS * STALL_LEN,
  wordLen: 0x02
}
export const MAP_EDIT = {
  area: 0x84,
  dbNumber: DB_DATA,
  start: 132,
  amount: 4,
  wordLen: 0x02
}
export const QUEUE_DELETE = {
  area: 0x84,
  dbNumber: DB_DATA,
  start: 140,
  amount: 4,
  wordLen: 0x02
}
export const REQ_0 = {
  area: 0x84,
  dbNumber: DB_DATA,
  start: 144,
  amount: 2,
  wordLen: 0x02
}
export const REQ_1 = {
  area: 0x84,
  dbNumber: DB_DATA,
  start: 146,
  amount: 2,
  wordLen: 0x02
}

/**
 * PLC SH
 */

export const PLC_SH = {
  ip: '140.80.20.13',
  rack: 0,
  slot: 2,
  polling_time: POLL_TIME
}
export const DB_DATA_INIT_AB_SH = 32
export const DB_DATA_INIT_EB_SH = 36
export const DB_DATA_INIT_MB_SH = 42
export const DATA_READ_SH = {
  area: 0x84,
  dbNumber: 505,
  start: 0,
  amount: 49,
  wordLen: 0x02
}
