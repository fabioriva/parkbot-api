const { parseISO, isEqual/*, subDays */ } = require('date-fns')
const { format/*, utcToZonedTime */ } = require('date-fns-tz')
const { byRange, daily, weekly, monthly, yearly } = require('./operations')
...
class History {
  ...
  async getOperations (query) {
    ...
    if (isEqual(dateFrom, dateTo)) {
      const operations = await daily(dateFrom, this.collection)
      return operations
    } else {
      const operations = await byRange(dateFrom, dateTo, this.collection)
      return operations
    }
  }
