const { addHours, parseISO, isEqual } = require('date-fns')
const { format/*, utcToZonedTime */ } = require('date-fns-tz')
const { byDevice, byRange, daily, weekly, monthly, yearly } = require('./operations')
...
class History {
  ...
  async getOperations (query) {
    ...
    if (query.dateString !== undefined) {
      ...
      // console.log('getOperations', date)
    ...
    const dateFrom = new Date(query.dateFrom)
    const dateTo = new Date(query.dateTo)
    console.log('getOperations', dateFrom, dateTo)
  ...
  async getOperationsDaily (query) {
    const date = new Date(query.dateString)
    const docs = await daily(date, this.collection)
    return [docs]
  }
  ...
  async getRecentActivity (limit) {
    ...
    const pipeline = match.concat(this.pipeline)
    const docs = await this.collection.aggregate(pipeline).toArray()
    // const docs = await this.collection.aggregate(this.pipeline).sort({ _id: -1 }).limit(limit).toArray()
    return {
      count: docs.length,
      documents: docs
    }
