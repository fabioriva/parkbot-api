import { addHours, parseISO, isEqual } from 'date-fns'
import { format /*, toZonedTime */ } from 'date-fns-tz'
import {
  byCard,
  byDevice,
  byRange,
  daily,
  weekly,
  monthly,
  yearly
} from './operations.js'

class Log {
  constructor (s7log) {
    this.alarm = s7log.alarm
    this.card = s7log.card
    this.date = new Date(format(s7log.date, "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"))
    this.device = s7log.device
    //  this.event = s7log.event
    this.mode = s7log.mode
    this.operation = s7log.operation
    this.size = s7log.size
    this.stall = s7log.stall
  }
}

class History {
  constructor (db) {
    // this.collection = db.collection('history')
    this.collection = db.collection('logs')
    // this.pipeline = [
    //   {
    //     $lookup: {
    //       from: 'alarms',
    //       localField: 'alarm',
    //       foreignField: 'id',
    //       as: 'alarm'
    //     }
    //   },
    //   { $unwind: { path: '$alarm', preserveNullAndEmptyArrays: true } },
    //   {
    //     $lookup: {
    //       from: 'devices',
    //       localField: 'device',
    //       foreignField: 'id',
    //       as: 'device'
    //     }
    //   },
    //   { $unwind: '$device' },
    //   {
    //     $lookup: {
    //       from: 'modes',
    //       localField: 'mode',
    //       foreignField: 'id',
    //       as: 'mode'
    //     }
    //   },
    //   { $unwind: '$mode' },
    //   {
    //     $lookup: {
    //       from: 'operations',
    //       localField: 'operation',
    //       foreignField: 'id',
    //       as: 'operation'
    //     }
    //   },
    //   { $unwind: '$operation' },
    //   { $unset: ['_id', 'alarm._id', 'device._id', 'mode._id', 'operation._id'] }
    //   // { $sort: { date: -1 } }
    // ]
    this.pipeline = [
      {
        $lookup: {
          from: 'alarms',
          let: { alarmId: '$alarm' },
          pipeline: [
            { $match: { $expr: { $eq: ['$id', '$$alarmId'] } } },
            { $project: { _id: 0 } },
            { $limit: 1 }
          ],
          as: 'alarm'
        }
      },
      { $set: { alarm: { $first: '$alarm' } } },

      {
        $lookup: {
          from: 'devices',
          let: { deviceId: '$device' },
          pipeline: [
            { $match: { $expr: { $eq: ['$id', '$$deviceId'] } } },
            { $project: { _id: 0 } },
            { $limit: 1 }
          ],
          as: 'device'
        }
      },
      { $set: { device: { $first: '$device' } } },

      {
        $lookup: {
          from: 'modes',
          let: { modeId: '$mode' },
          pipeline: [
            { $match: { $expr: { $eq: ['$id', '$$modeId'] } } },
            { $project: { _id: 0 } },
            { $limit: 1 }
          ],
          as: 'mode'
        }
      },
      { $set: { mode: { $first: '$mode' } } },

      {
        $lookup: {
          from: 'operations',
          let: { opId: '$operation' },
          pipeline: [
            { $match: { $expr: { $eq: ['$id', '$$opId'] } } },
            { $project: { _id: 0 } },
            { $limit: 1 }
          ],
          as: 'operation'
        }
      },
      { $set: { operation: { $first: '$operation' } } },

      { $project: { _id: 0 } }
    ]
  }

  // async get (query) {
  //   // console.log(query)
  //   // console.log(new Date(query.dateFrom), new Date(query.dateTo))
  //   // console.log(parseISO(query.dateFrom), parseISO(query.dateTo))
  //   const match = [{
  //     $match: {
  //       date: {
  //         $gte: addHours(parseISO(query.dateFrom), 1),
  //         $lt: addHours(parseISO(query.dateTo), 1)
  //       }
  //     }
  //   },
  //   { $sort: { date: -1 } },
  //   { $limit: 999 }]
  //   const pipeline = match.concat(this.pipeline)
  //   const docs = await this.collection.aggregate(pipeline).toArray()
  //   return docs
  // }

  async get (query) {
    const card = parseInt(query.card) || 0
    const device = parseInt(query.device) || 0
    const stall = parseInt(query.stall) || 0
    const page = parseInt(query.page) || 1
    const limit = parseInt(query.limit) || 20
    const skip = (page - 1) * limit
    // console.log(page, limit, skip)
    const pipeline = [
      {
        $match: {
          date: {
            $gte: addHours(parseISO(query.dateFrom), 1),
            $lt: addHours(parseISO(query.dateTo), 1)
          },
          card: card === 0 ? { $gte: 0 } : { $eq: card },
          device: device === 0 ? { $gte: 0 } : { $eq: device },
          stall: stall === 0 ? { $gte: 0 } : { $eq: stall }
        }
      },

      { $sort: { _id: -1 } },

      {
        $facet: {
          data: [
            { $skip: skip },
            { $limit: limit },

            {
              $lookup: {
                from: 'alarms',
                let: { alarmId: '$alarm' },
                pipeline: [
                  { $match: { $expr: { $eq: ['$id', '$$alarmId'] } } },
                  { $project: { _id: 0 } },
                  { $limit: 1 }
                ],
                as: 'alarm'
              }
            },
            { $set: { alarm: { $first: '$alarm' } } },

            {
              $lookup: {
                from: 'devices',
                let: { deviceId: '$device' },
                pipeline: [
                  { $match: { $expr: { $eq: ['$id', '$$deviceId'] } } },
                  { $project: { _id: 0 } },
                  { $limit: 1 }
                ],
                as: 'device'
              }
            },
            { $set: { device: { $first: '$device' } } },

            {
              $lookup: {
                from: 'modes',
                let: { modeId: '$mode' },
                pipeline: [
                  { $match: { $expr: { $eq: ['$id', '$$modeId'] } } },
                  { $project: { _id: 0 } },
                  { $limit: 1 }
                ],
                as: 'mode'
              }
            },
            { $set: { mode: { $first: '$mode' } } },

            {
              $lookup: {
                from: 'operations',
                let: { opId: '$operation' },
                pipeline: [
                  { $match: { $expr: { $eq: ['$id', '$$opId'] } } },
                  { $project: { _id: 0 } },
                  { $limit: 1 }
                ],
                as: 'operation'
              }
            },
            { $set: { operation: { $first: '$operation' } } },

            { $project: { _id: 0 } }
          ],

          total: [{ $count: 'count' }]
        }
      }
    ]
    // pipeline = pipeline.filter(Boolean)
    const result = await this.collection.aggregate(pipeline).toArray()
    const data = result[0].data
    const total = result[0].total[0]?.count || 0
    return { data, hasMore: skip + data.length < total, total }
  }

  async getLog (id) {
    const match = [{ $match: { _id: id } }]
    const pipeline = match.concat(this.pipeline)
    const docs = await this.collection.aggregate(pipeline).toArray()
    return docs[0]
  }

  async getCards (query) {
    const dateFrom = new Date(query.dateFrom)
    const dateTo = new Date(query.dateTo)
    const docs = await byCard(dateFrom, dateTo, this.collection)
    return docs
  }

  async getDevices (query, devices) {
    const dateFrom = new Date(query.dateFrom)
    const dateTo = new Date(query.dateTo)
    const docs = await byDevice(
      dateFrom,
      dateTo,
      devices.map((d) => d.name),
      this.collection
    )
    return docs
  }

  async getOperations (query) {
    // console.log(query)
    if (query.dateString !== undefined) {
      const date = new Date(query.dateString)
      const docs = await Promise.all([
        daily(date, this.collection),
        weekly(date, this.collection),
        monthly(date, this.collection),
        yearly(date, this.collection)
      ])
      return docs
    }
    // by range
    const dateFrom = new Date(query.dateFrom)
    const dateTo = new Date(query.dateTo)
    if (isEqual(dateFrom, dateTo)) {
      const docs = await daily(dateFrom, this.collection)
      return docs
    } else {
      const docs = await byRange(dateFrom, dateTo, this.collection)
      return docs
    }
  }

  async getRecentActivity (limit) {
    // const match = [{
    //   $match: {
    //     _id: { $exists: true }
    //   }
    // },
    // { $sort: { date: -1 } },
    // { $limit: limit }]
    // const pipeline = match.concat(this.pipeline)
    // const docs = await this.collection.aggregate(pipeline).toArray()
    const pipeline = [{ $sort: { _id: -1 } }, { $limit: limit }].concat(
      this.pipeline
    )
    const docs = await this.collection.aggregate(pipeline).toArray()
    return {
      count: docs.length,
      documents: docs
    }
  }

  async saveAction (log) {
    const res = await this.collection.insertOne({
      ...log,
      alarm: 0,
      device: 0,
      mode: 0,
      size: 0
    })
    await this.getLog(res.insertedId)
    // const doc = await this.getLog(res.insertedId)
    // console.log(doc)
    // return doc
  }

  async saveLog (s7log) {
    const log = new Log(s7log)
    const res = await this.collection.insertOne(log)
    const doc = await this.getLog(res.insertedId)
    return doc
  }
}

export default History
