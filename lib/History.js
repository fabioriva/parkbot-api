...
class History {
  ...
  async get (query) {
    console.log(query, parseInt(query.card), parseInt(query.stall), query.card === 0, parseInt(query.card) === 0)
    const card = parseInt(query.card)
    const stall = parseInt(query.stall)
    ...
    const pipeline = [
      {
        $match: {
          ...
          //card: query.card === 0 ? { $gte: 0 } : parseInt(query.card),
          //stall: query.stall === 0 ? { $gte: 0 } : parseInt(query.stall),
          card: card === 0 ? { $gte: 0 } : { $eq: card },
          stall: stall === 0 ? { $gte: 0 } : { $eq: stall },
        }
      },

      // ordinamento stabile come richiesto
      { $sort: { _id: -1 } },
  ...
  async getCards (query) {
    ...
    const docs = await byCard(dateFrom, dateTo, this.collection)
    return docs
  }
  ...
  async getRecentActivity (limit) {
    ...
    // const pipeline = match.concat(this.pipeline)
    // const docs = await this.collection.aggregate(pipeline).toArray()
    const pipeline = [{ $sort: { _id: -1 } }, { $limit: limit }].concat(
      this.pipeline
    )
    const docs = await this.collection.aggregate(pipeline).toArray()
    return {
  ...
  async saveAction (log) {
    ...
    await this.getLog(res.insertedId)
