...
class PLC extends EventEmitter {
  ...
  async log (data, def) {
    ...
    const bufferIsEmpty = buffer.every(b => b === 0)
    if (!bufferIsEmpty) {
      console.log(buffer)
    ...
    // const plc_ = this
    // console.log(typeof plc_.client, plc_.client)

    const empty = Buffer.alloc(LOG_LEN)
    console.log(empty)
    const res = await WriteArea(this.client, 0x84, def.DATA_READ.dbNumber, 0, LOG_LEN, 0x02, empty)
    // const res = await WriteArea(plc_.client, 0x84, def.DB_DATA, 0, LOG_LEN, 0x02, empty)
    // const res = await plc_.client.WriteArea(0x84, def.DB_DATA, 0, LOG_LEN, 0x02, )
    console.log(res)
  }
  ...
  async main (def, obj) {
    try {
      ...
      const plcDT = new Date(format(msec, "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"))
      obj.plcDT = plcDT
