...
class PLC {
  ...
  async alarms (def, obj, opts = {}) {
    try {
      ...
      this.app.publish('aps/alarms', obj.alarms)
    }
  }
  ...
  async cards (def, obj) {
    if (def.CARD_READ !== undefined) {
      try {
        ...
        this.app.publish('aps/cards', cards)
      } catch (e) {
        this.error(e)
      }
    }
  ...
  async log (buffer, def, obj) {
    ...
    if (log.etx === 0x03 && log.stx === 0x264) {
      ...
      if (res) {
        ...
        this.notifications.send(def.APS, doc)
      }
    }
  }
  ...
  async map (def, obj) {
    if (def.MAP_READ !== undefined) {
      try {
        ...
        this.app.publish('aps/map', obj.map)
      } catch (e) {
        this.error(e)
      }
    }
    ...
    try {
      ...
      if (stall !== undefined) {
        ...
        if (card !== undefined) {
          ...
          this.app.publish('aps/cards', obj.cards)
        }
      }
    } catch (e) {
      this.error(e)
  ...
  async main (def, obj) {
    try {
      ...
      this.app.publish('aps/overview', obj.overview)
      obj.racks.forEach(
        (rack, key) =>
          rack?.rack && this.app.publish('aps/racks/' + key, rack.rack)
      )
    }
  }
