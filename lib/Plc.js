...
class PLC {
  ...
  async notifications (aps, doc) {
    // if (doc.operation.id === 1) {
    console.log(doc)
    const { alarm, device, operation } = doc
    const recipients = this.notifications.getRecipients()
    const url = `http://${process.env.PRODUCTION_SERVER}:3000/api/notifications`
    console.log(url, process.env.NOTIFICATIONS_API_TOKENS[aps])
