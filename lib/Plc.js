...
// import Snap7Driver from './Snap7Driver.js'
...
class PLC {
  ...
  async dt (buffer, obj) {
    ...
    // obj.plcDT = new Date(getPlcDateTime(buffer.readInt16BE(20), buffer.readInt32BE(22)))
    obj.plcDT = new Date(format(getPlcDateTime(buffer.readInt16BE(20), buffer.readInt32BE(22)), "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"))
    // console.log(obj.plcDT)
  }
  ...
  async log (buffer, def, obj) {
    ...
    if (log.etx === 0x03 && log.stx === 0x264) {
      ...
      if (res) {
        ...
        this.app.publish('aps/info', JSON.stringify({ notification: doc }))
        this.notifications_(def.APS, doc)
      }
    }
  }
  ...
  async notifications_ (aps, doc) {
    // if (doc.operation.id === 1) {
    console.log(doc)
    const { alarm, device, operation } = doc
    const recipients = this.notifications.getRecipients()
    const url = process.env.NOTIFICATIONS_URL
    console.log(url, process.env.NOTIFICATIONS_API_TOKENS, process.env.NOTIFICATIONS_API_TOKENS.daman)
    ...
    const res = await fetch(url, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${process.env.NOTIFICATIONS_API_TOKENS.aps}`,
  ...
  async main (def, obj) {
    try {
      ...
      // this.notifications(def.APS, doc)
