...
function getPlcDateTime (days, msec) {
  return new Date(1990, 0, 1 + days, 0, 0, 0, msec).getTime()
}
...
class PLC {
    ...
    if (def.CARD_READ !== undefined) {
      try {
        ...
        const buffer = this.online
          ...
          ? await this.client.ReadArea(area, dbNumber, start, amount, wordLen)
          : Buffer.alloc(amount)
  ...
  async log (buffer, def, obj) {
    ...
    if (log.etx === 0x03 && log.stx === 0x264) {
      const res = await this.client.WriteArea(
        // this.client,
        def.DATA_READ.area,
