...
class PLC extends EventEmitter {
  ...
  //       if (opts.id) {
  //         await Promise.all(
  //           obj.alarms.filter(a => a.id === opts.id).map(async (item, index) => {
  ...
  //         // const alarms = obj.alarms.find(a => a.id === opts.id)
  //         // console.log(opts.id)
  //         // if (alarms) {
  //         //   console.log(opts.id, 'DB NR°', def.DBS_ALARM[opts.id - 1])
  //         //   const buffer = this.online
  //         //     ? await ReadArea(
  //         //       this.client,
  //         //       0x84,
  //         //       def.DBS_ALARM[opts.id - 1],
  //         //       def.DB_ALARM_INIT,
  //         //       def.DB_ALARM_LEN,
  //         //       0x02
  //         //     )
  //         //     : Buffer.alloc(def.DB_ALARM_LEN)
  //         //   obj.alarms[opts.id - 1] = await updateAlarms(0, buffer, def.ALARM_LEN, obj.alarms[opts.id - 1])
  //         //   // console.log(opts.id, alarms.alarms[0])
  //       } else {
  ...
  //             // obj.devices[index].alarms = item._active
  //           })
  //         )
  //       }
  //     } catch (e) {
  ...
  async main (def, obj) {
    try {
      ...
      // obj.motors !== undefined && obj.motors.map(m => m.update()) // update motors
      // obj.devices.forEach(
      //   (device) => device.views.forEach(
      //     (view) => view.bits !== undefined && view.bits.forEach(bit => bit.update())))
    } catch (e) {
      this.error(e)
    } finally {
