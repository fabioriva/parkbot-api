...
const { /* getPlcDateTime, ReadArea, */WriteArea } = require('./utils7')
...
class Router {
  ...
  dashboard (def, obj, prefix) {
    this.app.get(prefix + '/dashboard', async (res, req) => {
      ...
      // const ping1 = process.hrtime()
      const activity = await this.history.getRecentActivity(5)
      // const pong1 = process.hrtime(ping1)
      // console.log('activity execution time in millisecond is: ', (pong1[0] * 1000000000 + pong1[1]) / 1000000)

      // const ping2 = process.hrtime()
      const occupancy = obj.map.occupancy
      // const pong2 = process.hrtime(ping2)
      // console.log('occupancy execution time in millisecond is: ', (pong2[0] * 1000000000 + pong2[1]) / 1000000)

      // const ping3 = process.hrtime()
      // const operations = await this.history.getOperations({ dateString: format(new Date(), 'yyyy-MM-dd') })
      const operations = await this.history.getOperationsDaily({ dateString: format(new Date(), 'yyyy-MM-dd') })
      // const pong3 = process.hrtime(ping3)
      // console.log('operations execution time in millisecond is: ', (pong3[0] * 1000000000 + pong3[1]) / 1000000)

      // const ping4 = process.hrtime()
      const system = obj.devices.map((device) => {
        const clone = Object.assign({}, device)
        delete clone.views
        return clone
      })
      // const pong4 = process.hrtime(ping4)
      // console.log('system execution time in millisecond is: ', (pong4[0] * 1000000000 + pong4[1]) / 1000000)
      sendJson(res, {
        activity,
        occupancy,
        operations,
