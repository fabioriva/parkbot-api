...
async function setPlcClock (client, date) {
  if (!(date instanceof Date) || !Number.isFinite(date.getTime())) {
    throw new TypeError('Data non valida')
  }
  // Check write status
  const pending = await ReadArea(client, 0x84, 6, 30, 1, 0x02)
  if (pending[0] !== 0) {
    throw new Error('Richiesta di impostazione già presente nel PLC')
  }
  // Codifica DTL in UTC.
  const data = Buffer.alloc(12)
  data.writeUInt16BE(date.getUTCFullYear(), 0)
  data[2] = date.getUTCMonth() + 1
  data[3] = date.getUTCDate()
  data[4] = date.getUTCDay() + 1 // Siemens: domenica = 1
  data[5] = date.getUTCHours()
  data[6] = date.getUTCMinutes()
  data[7] = date.getUTCSeconds()
  data.writeUInt32BE(date.getUTCMilliseconds() * 1_000_000, 8)

  // Prima trasferisci la data completa, poi attiva la richiesta.
  await WriteArea(client, 0x84, 6, 16, 12, 0x02, data)
  await WriteArea(client, 0x84, 6, 30, 1, 0x02, Buffer.from([1]))

  const deadline = Date.now() + 5000

  while (Date.now() < deadline) {
    // Byte 28–29: WriteStatus; byte 30: SetRequest.
    const reply = await ReadArea(client, 0x84, 6, 28, 3, 0x02)
    // console.log(reply);
    console.log({
      writeStatus: `0x${reply.readUInt16BE(0).toString(16).padStart(4, '0')}`,
      setRequest: reply[2]
    })

    if (reply[2] === 0) {
      const status = reply.readUInt16BE(0)
      if (status !== 0) {
        throw new Error(
          `WR_SYS_T: errore 0x${status.toString(16).padStart(4, '0')}`
        )
      }
      return
    }
  ...
  throw new Error('Timeout: esito della richiesta non confermato')
}

class Router {
  constructor (app, history, notifications, plc) {
    this.app = app
  ...
  cards (def, obj, prefix) {
    this.app.get(prefix + '/cards', async (res, req) => {
      ...
      if (!session) {
        return sendJson(res, { error: 'Unauthorized' }, '401 Unauthorized')
      }
    ...
    this.app.post(prefix + '/card/edit', async (res, req) => {
      ...
      if (regexp.test(code) || code === -1) {
        ...
        await this.history.saveAction({
          card,
          date: obj.plcDT,
          operation: 4, // 'op-change-pin'
          stall: 0,
  ...
  history_ (def, obj, prefix) {
    this.app.get(prefix + '/history', async (res, req) => {
      ...
      const devices = obj.devices.map(({ id, name }) => ({ id, name }))
      devices.unshift({ id: 0, name: 'All' })
      const history = await this.history.get(query)
      sendJson(res, {
        devices,
  ...
  map (def, obj, prefix) {
    ...
    this.app.post(prefix + '/map/edit', async (res, req) => {
      ...
      //     card,
      //     date: obj.plcDT,
      //     operation: card !== 0 ? 5 : 6, // 'op-stall-in' 'op-stall-out'
      //     stall,
      //     user: session.user.email
      //   })
  ...
  overview (def, obj, prefix) {
    ...
    this.app.post(prefix + '/operation/entry', async (res, req) => {
      ...
      const { area, dbNumber, start, amount, wordLen } =
        def.REQ_ENTRY[entry - 1]
  ...
  notifications_ (def, obj, prefix) {
    this.app.get(prefix + '/notifications', async (res, req) => {
      log(req)
      res.onAborted(() => {
        res.aborted = true
      })
      ...
      const recipients = await this.notifications.get()
      sendJson(res, { recipients })
    })
    this.app.post(prefix + '/notifications/add', async (res, req) => {
      log(req)
      res.onAborted(() => {
        res.aborted = true
      })
      ...
      const result = await this.notifications.add(json)
      sendJson(res, { result })
    })
    this.app.post(prefix + '/notifications/delete', async (res, req) => {
      log(req)
      res.onAborted(() => {
        res.aborted = true
      })
      ...
      const result = await this.notifications.delete(json)
      sendJson(res, { result })
    })
    this.app.post(prefix + '/notifications/update', async (res, req) => {
      log(req)
      res.onAborted(() => {
        res.aborted = true
      })
      ...
      const result = await this.notifications.update(json)
      sendJson(res, { result })
    })
    this.app.get(prefix + '/recipients', async (res, req) => {
      log(req)
      res.onAborted(() => {
        res.aborted = true
      })
  ...
  run (def, obj) {
    ...
    this.app.get(prefix + '/set-now', async (res, req) => {
      log(req)
      res.onAborted(() => {
        res.aborted = true
      })
      ...
      // const { card, index } = json
      const now = new Date()
      await setPlcClock(this.plc.client, now)
      console.log('Orologio PLC aggiornato')
      sendJson(res, { now })
    })
    ...
    this.notifications_(def, obj, prefix)
