...
import { readJson, safeRoute, sendJson, Message } from './json.js'
...
class Router {
  ...
  map (def, obj, prefix) {
    ...
    //     headers[key] = value
    //   })
    //   // 3. Now run async logic safely
    //   ;(async () => {
    //     // const json = await readJson(res)
    //     // if (res.aborted || json === null) return
    //     const session = await checkAuth(headers)
    //     if (res.aborted) return
    //     if (!session) {
    //       if (!res.aborted) sendJson(res, { error: 'Unauthorized' }, '401 Unauthorized')
    //       return
    //     }
    //     if (!res.aborted) {
    ...
    //   // sendJson(res, obj.map)
    // })
    this.app.post(prefix + '/map/edit', safeRoute(async (req, res) => {
      const json = await readJson(res)
      if (res.aborted || json === null) return

      const session = await checkAuth(req.headers)
      if (!session) {
        return sendJson(res, { error: 'Unauthorized' }, '401 Unauthorized')
      }

      return sendJson(res, { severity: 'success', message: 'Hello' })
    }))
    ...
    this.app.post(prefix + '/map/edit', async (res, req) => {
      ...
      ;(async () => {
        ...
        if (!res.aborted) {
          sendJson(res, { severity: 'success', message: 'Hello' })
        }
      })()
